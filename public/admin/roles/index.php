<?php
    ...
    // 1. IMPORTANTE: Iniciar sesión ANTES de cualquier verificación
    if (session_status() === PHP_SESSION_NONE) {
        session_start();
    }

    // 2. Verificar seguridad (Ahora sí funcionará porque la sesión ya existe)
        ...
        if(!empty($nombre)) {
            try {
                if($accion == 'create') {
                    ...
                    $_SESSION['temp_msg'] = "Rol agregado correctamente.";
                    $_SESSION['temp_msg_type'] = "success";

                } elseif($accion == 'update') {
                    // UPDATE
                    $id = (int) $_POST['id'];
                    $stmt = $pdo->prepare("UPDATE ROLES SET NAME = :name WHERE ID_ROLE = :id");
                    $stmt->execute([':name' => $nombre, ':id' => $id]);

                    $_SESSION['temp_msg'] = "Rol actualizado correctamente.";
                    $_SESSION['temp_msg_type'] = "success";
                }
    ...
    if(isset($_GET['delete_id'])) {
        ...
        try {
            $stmt = $pdo->prepare("DELETE FROM ROLES WHERE ID_ROLE = :id");
            $stmt->execute([':id' => $id]);

            $_SESSION['temp_msg'] = "Rol eliminado.";
            $_SESSION['temp_msg_type'] = "success";
...
<div class="mb-5"> 
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="fw-bold"><i class="bi bi-shield-lock"></i> Gestión de Roles</h2>
            ...
            <form method="POST" action="">
                <div class="modal-body py-4">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control form-control-lg" name="nombre" id="add_nombre" required placeholder="Ej. Administrador" autocomplete="off">
                    </div>
                </div>
        ...
        <div class="modal-content">
            ...
            <form method="POST" action="">
                <div class="modal-body py-4">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold">Nombre del Rol:</label>
                        <input type="text" class="form-control form-control-lg" name="nombre" id="role_nombre" required autocomplete="off">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
...
<script>
    ...
    editModal.addEventListener('show.bs.modal', function (event) {
        ...
        var modalInputId = editModal.querySelector('#role_id');
        var modalInputNombre = editModal.querySelector('#role_nombre');
    ...
    function confirmDelete(e, url) {
        ...
        Swal.fire({
            title: '¿Estás seguro?',
            text: "No podrás revertir esto. Se eliminará el rol.",
            icon: 'warning',
